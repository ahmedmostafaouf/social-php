...
<div class="container-fluid">
    ...
    <div class=" col-xs-12 col-sm-12 col-md-6 col-lg-6" >
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    $pharase =$_POST['pharase'];
                    $stmt = $con->prepare("
                    SELECT *  
                    FROM `user` 
                    WHERE (CONVERT(`user_id` USING utf8) LIKE '%$pharase%' 
                    OR CONVERT(`f_name` USING utf8) LIKE '%$pharase%'
                     OR CONVERT(`l_name` USING utf8) LIKE '%$pharase%'
                      OR CONVERT(`email` USING utf8) LIKE '%$pharase%')");
                    $stmt->execute();
                    $infos = $stmt->fetchAll();
          }else{
